...
- **×** state/active-task.md / state/decisions.md の内容を重複して保存する（ファイルは compaction で消えない、再 Read すれば同じものが読める）
...
[補助] PreCompact hook（dialogue + evidence 保全）
  ...
  → PostCompact hook は使わない（state/active-task.md 等は compaction で消えないので注入不要）

[根本] 作業分割
  → /clear → state/active-task.md読み込み → 続行
...
### state/active-task.md 更新タイミング（必須）

以下のタイミングで `.claude/state/active-task.md` を更新すること:
...
state/active-task.mdは**30行以内**に保つ。完了済みは詳細省略。

### state/decisions.md — 確定事項ログ

ユーザーと合意した決定事項を `.claude/state/decisions.md` に記録すること。specをまたいで残る「これまでに決まったこと」の台帳。

- specは完了時にクローズされる。確定事項はspec完了後も有効なので、specに混ぜない
- 新しい確定事項が会話で生まれたら、その場でstate/decisions.mdに追記する
...
- state/active-task.mdから「確定事項: state/decisions.md 参照」でポインタを張る

## compaction後の復帰

`/clear` やcompaction後に文脈を失った場合:

1. `.claude/state/active-task.md` を読む
2. `.claude/state/decisions.md` を読む（確定事項の把握）
3. 記載された参照ドキュメントを読む
...
- 引数なし → `state/roadmap.md` を表示
- `/anchor roadmap init` → ユーザーとフェーズ分けを合意し、`state/roadmap.md` を初期化
- `/anchor roadmap done <phase#>` → 指定フェーズを done に更新し、完了日を記録
- `/anchor roadmap activate <phase#>` → 指定フェーズを active に更新。同時に state/active-task.md の「現在のフェーズ」を更新
...
1. **ロードマップ作成**: PJ全体のフェーズ分けをユーザーと合意し、`/anchor roadmap init` で `state/roadmap.md` に書く
...
1. タスクの規模を判定（小/中/大）
   - **小**（バグ修正、typo、1ファイル修正）→ spec なし。state/active-task.md 更新のみで着手
   - **中**（機能追加、調査、複数ファイル変更）→ `/anchor create <name>` で軽量 spec 作成 → 着手
   - **大**（新機能設計、アーキテクチャ変更、戦略立案）→ `/anchor create <name>` でフル spec 作成 → **ユーザーレビュー・合意** → 着手
2. `.claude/state/active-task.md` を更新（参照 spec、チェックリスト）
3. 作業に着手
4. 項目完了ごとに state/active-task.md を更新

**spec なしでの着手は禁止。** 「小さそうだから」で始めて中規模化した場合は、その場で spec を書き起こす。

### 確定事項の記録

ユーザーと合意した決定事項は `.claude/state/decisions.md` に記録する。

- specは完了時にクローズされるが、確定事項はspec完了後も有効
- 新しい確定事項が会話で生まれたら、その場でstate/decisions.mdに追記する
- specを凍結・立て直す時は、確定として残る事項をstate/decisions.mdに移す
- state/active-task.mdの「確定事項」欄は「state/decisions.md 参照」のポインタのみ

### フィードバック受領時のフロー

1. フィードバックを項目化する
2. `/anchor update <name>` で該当 spec を**アクション前に**更新
3. 確定事項が含まれていれば `.claude/state/decisions.md` に追記
4. `.claude/state/active-task.md` を更新（チェックリスト形式）
5. アクションに着手
6. 項目完了ごとに state/active-task.md を更新
...
4. `.claude/state/active-task.md` を更新

### state/active-task.md の管理

作業開始時・中断時・完了時に `.claude/state/active-task.md` を更新する。
...
### 生成物6.5: state/roadmap.md（初期テンプレート）
...
### 生成物7: state/active-task.md（初期テンプレート）
...
## 確定事項
state/decisions.md 参照
```

### 生成物8: state/decisions.md（初期テンプレート）
...
- [ ] state/active-task.md を初期テンプレートで作成
- [ ] state/decisions.md を初期テンプレートで作成
