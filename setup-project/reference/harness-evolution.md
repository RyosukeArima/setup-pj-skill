...
**初期状態では全PJ共通のstate/active-task.mdチェックのみ**。ドメイン固有チェックは運用中にユーザー承認を得て追加する。
...
LOG="$DIR/state/harness-log.jsonl"
...
# チェック1: state/active-task.md の状態整合性（構造のみ・メッセージ解析なし）
# フォーマット: "## 状況: 進行中" のようにインライン記載（rules/active-task-lifecycle.md で規定）
if [ -f "$DIR/state/active-task.md" ]; then
  if ! grep -q "最初のタスク着手時に更新" "$DIR/state/active-task.md" 2>/dev/null; then
    STATUS_LINE=$(grep '^## 状況' "$DIR/state/active-task.md" 2>/dev/null | head -1 || echo "")
    UNCHECKED=$(grep -c '^- \[ \]' "$DIR/state/active-task.md" 2>/dev/null || true)
    UNCHECKED=${UNCHECKED:-0}
    CHECKED=$(grep -c '^- \[x\]' "$DIR/state/active-task.md" 2>/dev/null || true)
    CHECKED=${CHECKED:-0}

    # 不整合1: 状況が「完了」なのに未チェック項目がある
    if echo "$STATUS_LINE" | grep -q "完了"; then
      if [ "$UNCHECKED" -gt 0 ]; then
        ISSUES="${ISSUES}- state/active-task.md: 状況「完了」ですが未完了項目が${UNCHECKED}件あります\n"
      fi
    fi
    ...
    if [ "$CHECKED" -gt 0 ] && [ "$UNCHECKED" -eq 0 ]; then
      if echo "$STATUS_LINE" | grep -q "進行中"; then
        ISSUES="${ISSUES}- state/active-task.md: 全項目完了ですが状況が「進行中」のままです\n"
      fi
    fi
  fi
fi
...
### 生成物2: state/harness-log.jsonl

空ファイルとして作成:

```bash
touch "$CLAUDE_PROJECT_DIR/.claude/state/harness-log.jsonl"
```
...
state/active-task.md の見出しに `## 状況: {値}` をインラインで記載する（stop guardian がパースする）。
...
- [ ] state/harness-log.jsonl を touch で作成
